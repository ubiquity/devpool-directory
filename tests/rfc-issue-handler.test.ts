...
process.env.DEVPOOL_REPO_NAME = "devpool-rfc";
process.env.RFC = "true";
...
import issueDevpoolTemplate from "../mocks/issue-devpool-template-rfc.json";
import issueTemplate from "../mocks/issue-template-rfc.json";
import { handleDevPoolIssue, createDevPoolIssue } from "../helpers/github";

const DEVPOOL_OWNER_NAME = "ubiquity";
const DEVPOOL_REPO_NAME = "devpool-rfc";
...
describe("handleDevPoolIssue", () => {
  ...
  describe("Devpool Directory", () => {
    ...
    test("updates issue labels in devpool when project issue labels change", async () => {
      const devpoolIssue = {
        ...issueDevpoolTemplate,
        labels: [{ name: "Partner: ubiquity/test-repo" }, { name: "id: 2" }, { name: "Time: 1h" }],
      } as GitHubIssue;
    ...
    // cause: (!hasCorrectPriceLabel) && devpoolIssue.state == "open"
    // comment: "Closed (has price labels):"
    test("closes devpool issue when project issue has price labels", async () => {
      const devpoolIssue = {
        ...issueDevpoolTemplate,
      } as GitHubIssue;
      ...
      expect(logSpy).toHaveBeenCalledWith(`Updated state: (Closed (has price labels))\n${updatedIssue.html_url} - (${partnerIssue.html_url})`);
    });
    ...
    test("does not add the Unavailable to an open devpool issue, _ever_", async () => {
      const devpoolIssue = {
        ...
        number: 1,
        node_id: "1",
        repository_url: `https://github.com/ubiquity/${DEVPOOL_REPO_NAME}`,
        html_url: `https://github.com/ubiquity/${DEVPOOL_REPO_NAME}/issues/1`,
      } as GitHubIssue;
  ...
  const HTML_URL = `https://github.com/not-ubiquity/${DEVPOOL_REPO_NAME}/issues/1`;
  const REPO_URL = `https://github.com/not-ubiquity/${DEVPOOL_REPO_NAME}`;
...
describe("createDevPoolIssue", () => {
  ...
  const twitterMap: { [key: string]: string } = {
    "ubiquity/test-repo": DEVPOOL_OWNER_NAME,
  };
  ...
  describe("Devpool Directory", () => {
    ...
    test("only creates a new devpool issue if price labels are NOT set, it's unassigned, opened and not already a devpool issue", async () => {
      const partnerIssue = {
        ...issueTemplate,
        assignee: null,
      } as GitHubIssue;
    ...
    test("does not create a new devpool issue if price labels are set", async () => {
      const partnerIssue = {
        ...issueTemplate,
        labels: [],
      } as GitHubIssue;
